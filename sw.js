// Service Worker for 时刻 (Shike) - v0.4.0 Final
var CACHE_NAME = 'shike-v040-final';
...
self.addEventListener('fetch',function(event){
  if(event.request.mode==='navigate'||event.request.destination==='document'||event.request.url.endsWith('.html')||event.request.url.endsWith('/')||event.request.url.endsWith('manifest.json')||event.request.url.endsWith('sw.js')){
  ...
  event.respondWith(caches.match(event.request).then(function(cached){
    ...
    return fetch(event.request).then(function(response){
      if(response&&response.status===200&&response.type==='basic'){var copy=response.clone();caches.open(CACHE_NAME).then(function(cache){cache.put(event.request,copy)})}
      return response;
