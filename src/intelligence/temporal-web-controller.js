(function(global,factory){
  var modules={
    ...
    backupAdapter:global.ShikeChronosBackupAdapter,inboxView:global.ShikeLifeInboxPreview,actionView:global.ShikeNextActionCard,legacyAdapter:global.ShikeLegacyRecordAdapter
  };
  if(typeof module!=='undefined'&&module.exports){modules={
    ...
    backupAdapter:require('./adapters/backup-adapter.js'),inboxView:require('./ui/life-inbox-preview.js'),actionView:require('./ui/next-action-card.js'),legacyAdapter:require('./adapters/legacy-record-adapter.js')
  };}
  ...
  function defaultApi(){return {getRecords:function(){return[];},saveRecord:function(){return null;},removeRecord:function(){},updateRecord:function(){return false;},clearInput:function(){},openDetail:function(){},notify:function(){},refresh:function(){}};}
  function create(options){
    options=options||{};var api=defaultApi();var temporalRepository=null;var graphRepository=null;var waitingRepository=null;var backupAdapter=null;var initialized=false;var renderToken=0;
    var waitingTombstones=new Map();
    var state={sourceText:'',drafts:[],rejected:[],persisting:false,error:'',saving:new Set(),ignored:new Set(),never:readNever(),lastModel:null};
    ...
    function updateDraft(id,changes){
      ...
      try{state.drafts[index]=modules.intelligence.updateDraft(state.drafts[index],changes);state.error='';renderPreview();temporalRepository.saveDrafts([state.drafts[index]]).catch(function(){state.error='修改未能保存，请重试。';renderPreview();});}catch(error){notify('草稿修改无效','error');}
    }
    ...
    function cancelDraft(id){
      ...
      temporalRepository.setStatus(id,'cancelled').catch(function(){return temporalRepository.removeDraft(id);}).then(function(){removeDraftFromState(id);}).catch(function(){state.saving.delete(id);state.error='取消操作未能保存。';renderPreview();});
    }
    ...
    async function confirmDraft(id){
      ...
      try{
        ...
        await temporalRepository.setStatus(id,'confirmed');state.saving.delete(id);removeDraftFromState(id);api.refresh();await renderInsights(api.getRecords());return true;
      }catch(error){
    ...
    async function renderInsights(records){
      ...
      try{
        ...
        var waitingQueries=modules.waitingEngine.queries(refreshed,new Date());if(token!==renderToken)return;
        state.lastModel={suggestions:suggestions,waiting:waitingQueries,graph:{nodeCount:graph.nodes.length,edgeCount:graph.edges.length}};
    ...
    async function handleSuggestionAction(action,recordId){
      ...
      if(action==='ignore'){state.ignored.add(recordId);await renderInsights(api.getRecords());return;}
      if(action==='never'){state.never.add(recordId);writeNever(state.never);await renderInsights(api.getRecords());return;}
      if(action==='later'){api.updateRecord(recordId,{postpone:true});notify('已按你的选择稍后处理。','success');api.refresh();await renderInsights(api.getRecords());return;}
      if(action==='complete'){
        api.updateRecord(recordId,{recordState:'completed'});var waiting=await waitingRepository.list();var item=waiting.find(function(entry){return entry.recordId===recordId;});if(item)await waitingRepository.upsert(modules.waitingEngine.transition(item,'resolved'));notify('已标记完成。','success');api.refresh();await renderInsights(api.getRecords());
      }
    }
    async function init(nextApi){
      api=Object.assign(defaultApi(),nextApi||{});temporalRepository=options.temporalRepository||modules.temporalRepository.create();graphRepository=options.graphRepository||modules.graphRepository.create();waitingRepository=options.waitingRepository||modules.waitingRepository.create();backupAdapter=options.backupAdapter||modules.backupAdapter.create({graphRepository:graphRepository,waitingRepository:waitingRepository});initialized=true;
    ...
    async function diagnostics(){return {initialized:initialized,pendingDrafts:state.drafts.length,graph:initialized?await graphRepository.snapshot():null,waiting:initialized?await waitingRepository.list():[],model:copy(state.lastModel)};}
    return Object.freeze({init:init,captureIfNeeded:captureIfNeeded,confirmDraft:confirmDraft,confirmAll:confirmAll,cancelDraft:cancelDraft,render:renderInsights,tombstoneRecord:tombstoneRecord,restoreRecord:restoreRecord,purgeRecord:purgeRecord,augmentBackup:augmentBackup,importBackupSidecars:importBackupSidecars,saveSnapshotSidecar:saveSnapshotSidecar,restoreSnapshotSidecar:restoreSnapshotSidecar,hasPendingDrafts:hasPendingDrafts,diagnostics:diagnostics});
  }
