(function(ns){
  function route(text){var t=String(text).replace(/[?？!！。.，,\s]+$/g,'').trim();var match;
  if(/^(今天有什么|看看今天|今日概览|今天怎么样)/.test(t))return {intent:'summarize_today',args:{}};
  if((match=t.match(/^(?:查一下|搜索|找一下|帮我找|查一查)\s*(.+)$/)))return {intent:'search_records',args:{query:match[1]}};
  ...
  // Create record detection via normalization layer
  if(window.ShikeSpriteCreateIntent){
    var norm=window.ShikeSpriteCreateIntent.normalize(text);
    if(norm.isCreate && norm.cleaned){
      return {intent:'create_record',args:{text:norm.cleaned,sourceText:norm.sourceText,hasTime:norm.hasTime}};
    }
  }
