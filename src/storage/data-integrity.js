(function(global){
  ...
  function normalizeRecord(raw,index,seen){
    ...
    var record={
      ...
      id:id,
      title:title,
      type:text(raw.type||raw.recordKind||'note',40),
      recordKind:text(raw.recordKind||raw.type||'note',40),
      dateKey:dateKey,
      time:timeValue,
      timeText:timeValue,
      repeat:repeat,
      note:text(raw.note,5000),
      sourceText:text(raw.sourceText||raw.rawText,5000),
      rawText:text(raw.rawText||raw.sourceText,5000),
      pinned:!!raw.pinned,
  ...
  function classify(records){
    ...
    (Array.isArray(records)?records:[]).forEach(function(raw,index){
      try{
        valid.push(normalizeRecord(raw,index,seen));
      }catch(error){
        quarantined.push({
