...
// Runtime tests for sprite create intent normalization (v2.3.0-alpha2-webfix)
const fs = require('fs');
...
add('parser-adapter.js hash unchanged', () => {
  ...
  add('Version is v2.3.0-alpha2-webfix', () => {
  const version = fs.readFileSync(path.join(root, 'src/config/version.js'), 'utf8');
    assert(version.includes("v2.3.0-alpha2-webfix"), 'version.js has v2.3.0-alpha2-webfix');
});
