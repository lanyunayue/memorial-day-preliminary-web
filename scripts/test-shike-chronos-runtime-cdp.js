...
async function main(){
  ...
  const confirmed=await client.evaluate(`(async()=>{const d=await ShikeChronosWeb.diagnostics();return {records:records.length,ids:[...new Set(records.map((record)=>record.id))].length,nodes:d.graph.nodes.length,edges:d.graph.edges.length,waiting:d.waiting.length,suggestions:d.model.suggestions.length,reason:d.model.suggestions[0]&&d.model.suggestions[0].reason,actionVisible:!!document.querySelector('#temporalActionBlock .temporal-action')};})()`);
  ...
  if(ARTIFACT_DIR){fs.mkdirSync(ARTIFACT_DIR,{recursive:true});const shot=await client.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:true});fs.writeFileSync(path.join(ARTIFACT_DIR,'chronos-vertical.png'),Buffer.from(shot.data,'base64'));}
  ...
  client.close();
  console.log('Chronos browser vertical passed: 18/18');
}
