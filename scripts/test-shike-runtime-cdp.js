...
async function main() {
  ...
  await client.evaluate(`
    ...
    localStorage.setItem('shike_seen_release_note_version', 'v0.9.7');
  `);
  await client.navigate(APP_URL);
  ...
  add('app loads v0.9.7 route shell', version.appVersion === 'v0.9.7' && version.cacheFetch === 'function' && version.route, JSON.stringify(version));
  ...
  `);
  add('v0.9.7 product polish surfaces work at runtime', polish.home.exampleHidden && polish.home.demoHidden && polish.home.hasInput && polish.home.hasToday && polish.swipe.hasWrapper && polish.swipe.hasEdit && polish.swipe.hasDanger && polish.my.demo && polish.my.feedback && polish.my.future && polish.release.shown && polish.release.bullets >= 5 && polish.sprite.bear && polish.sprite.today && polish.sprite.batch && polish.sprite.calendar && polish.sprite.backup && polish.sprite.update, JSON.stringify(polish));
