...
async function main(){
  ...
  console.log('Product validation captured state: '+JSON.stringify(Object.assign({},captured,{exportPayload:undefined})));
  ...
  const manager=await client.evaluate(`(()=>{const valid=${JSON.stringify(captured.exportPayload)};var validAccepted=false;var privacyRejected=false;var orphanRejected=false;try{ShikeSessionAnalyzer.assertExport(valid);validAccepted=true;}catch(error){}try{const unsafe=JSON.parse(JSON.stringify(valid));unsafe.sessions[0].note='complete private life text';ShikeSessionAnalyzer.assertExport(unsafe);}catch(error){privacyRejected=true;}try{ShikeStudyExporter.validateCodes({sessions:[]},[{participantCode:'TEST-BROWSER',sessionId:'rs_missing',issueCode:'copy_confusion',humanCoded:true}]);}catch(error){orphanRejected=true;}return {validAccepted:validAccepted,privacyRejected:privacyRejected,orphanRejected:orphanRejected,emptyReport:document.getElementById('metrics').textContent.includes('HUMAN PARTICIPANTS REQUIRED'),exportDisabled:document.getElementById('exportStudy').disabled};})()`);
  assert(manager.validAccepted&&manager.privacyRejected&&manager.orphanRejected,'participant manager accepted invalid evidence or rejected a canonical export');assert(manager.emptyReport&&manager.exportDisabled,'participant manager empty state is dishonest');
