...
add('left swipe classes exist', style.includes('.record-swipe.swiped') && script.includes('touchstart') && script.includes('touchmove'));
...
add('desktop more button exists', script.includes('record-more-btn') && script.includes('toggleRecordActions'));
...
add('mobile style exists', style.includes('touch-action:pan-y') && style.includes('translateX(-212px)'));
...
add('version is v1.0.0-rc', script.includes("APP_VERSION='v1.0.0-rc'"));
add('service worker cache is v097', sw.includes("CACHE_NAME = 'shike-v100rc-v45'"));
