...
async function main(){
  ...
  const writeResult=await client.evaluate(`(async()=>{
    ...
    const recordsAt1000=await window.ShikeIndexedDb.count('records');
    ...
    const counts={};for(const store of stores)counts[store]=await window.ShikeIndexedDb.count(store);const estimateAfter=navigator.storage&&navigator.storage.estimate?await navigator.storage.estimate():{};
  ...
  let workload;try{workload=await client.evaluate(`(async()=>{
    ...
    window.__chronosStressStage='next action';const [records,nodes]=await Promise.all([read('records'),read('temporal_nodes')]);const graph={nodes,edges:[]};let actionCount=0;for(let run=0;run<5;run++){const began=performance.now();actionCount=window.ShikeNextActionEngine.compute({records,graph,waiting:[],now:'2026-07-13T10:00:00+08:00'}).length;metrics.nextAction.push(performance.now()-began);}
    window.__chronosStressStage='weekly review';let weeklyCount=0;for(let run=0;run<5;run++){const began=performance.now();weeklyCount=window.ShikeWeeklyReview.generate({records,graph,waiting:[],corrections:[],now:'2026-07-13T10:00:00+08:00'}).openCommitments.length;metrics.weeklyReview.push(performance.now()-began);}
