...
const requiredStores=['records','settings','conversations','subscriptions','feed_items','migrations','audit_log','quarantined_records','portable_records','portable_envelopes'];
const temporalStores=['temporal_drafts','temporal_nodes','temporal_edges','temporal_waiting','temporal_corrections','temporal_meta','temporal_tombstones','temporal_operations','temporal_operation_quarantine','temporal_locks','temporal_adaptation_rules'];
const checks=[
 ['database name is stable',constants.includes("LOCAL_DB_NAME='shike_local_db'")],
 ['schema version is 5',constants.includes('LOCAL_DB_VERSION=5')],
 ['all required stores declared',requiredStores.every((store)=>source.includes(`'${store}'`))],
 ...
 ['portable import is cross-store transaction',source.includes("['records','portable_records','portable_envelopes','audit_log']")],
 ['portable import verifies post-write state',source.includes('portable_post_write_verification_failed')&&source.includes('portable_record_count_mismatch')&&source.includes('portable_envelope_verification_failed')],
 ['portable import supports fault rollback',source.includes('portable_import_fault_injected')&&source.includes('failAfterWrite')]
];
