...
async function ready(client){for(let index=0;index<200;index++){try{if(await client.evaluate("document.readyState==='complete'&&!!window.ShikeChronosWeb&&!!window.ShikeLocalFirst&&ShikeLocalFirst.getStatus().ready"))return;}catch(error){}await delay(100);}const state=await client.evaluate(`({url:location.href,ready:document.readyState,chronos:!!window.ShikeChronosWeb,localFirst:!!window.ShikeLocalFirst,status:window.ShikeLocalFirst&&ShikeLocalFirst.getStatus(),errors:document.body&&document.body.innerText.slice(0,200)})`).catch((error)=>({evaluationError:error.message}));throw new Error('page bootstrap timed out: '+JSON.stringify(state));}
...
(async function(){
  ...
  const source='周五前把实习材料交给老师';stage='capture shared draft';await clients[0].evaluate(`ShikeChronosWeb.captureIfNeeded(${JSON.stringify(source)})`);await delay(300);
  ...
  const draftIds=await Promise.all(clients.map((client)=>client.evaluate(`ShikeChronosWeb.diagnostics().then((item)=>{const cards=[...document.querySelectorAll('.temporal-draft')];return {pending:item.pendingDrafts,id:cards[0]&&cards[0].getAttribute('data-draft-id')};})`)));
  check('both tabs load one shared draft',draftIds.every((item)=>item.pending===1&&item.id===draftIds[0].id));
  stage='concurrent confirm';const confirmResults=await Promise.all(clients.map((client)=>client.evaluate(`ShikeChronosWeb.confirmDraft(${JSON.stringify(draftIds[0].id)})`)));await delay(500);stage='inspect durable state';
  check('only one tab commits the shared draft',confirmResults.filter(Boolean).length===1);
  const durable=await clients[0].evaluate(`Promise.all([ShikeIndexedDb.getAll('records'),ShikeIndexedDb.getAll('temporal_operations'),ShikeIndexedDb.getAll('temporal_edges'),ShikeIndexedDb.getAll('temporal_drafts')]).then(([r,o,e,d])=>({records:r.map((x)=>x.id),operations:o.map((x)=>({id:x.id,status:x.status})),edgeRecords:[...new Set(e.map((x)=>x.sourceRecordId))],draftStatus:d[0]&&d[0].status}))`);
  ...
  stage='resync both tabs';const reloadCounts=await Promise.all(clients.map((client)=>client.evaluate(`ShikeLocalFirst.bootstrap().then((result)=>{records=result.records.slice();return records.length;})`)));stage='check convergence';
  check('both tabs converge after durable resync',reloadCounts.every((count)=>count===1));

  const resource=`contract_${Date.now()}`;stage='create browser coordinators';await Promise.all(clients.map((client,index)=>client.evaluate(`window.__chronosMultiTab=ShikeTemporalMultiTab.create({lock:ShikeTemporalOperationLock.create(undefined,{ownerId:'browser_tab_${index}',ttl:5000})});true`)));
  const versions=await Promise.all(clients.map((client)=>client.evaluate(`__chronosMultiTab.version(${JSON.stringify(resource)})`)));check('browser tabs share version zero',versions[0]===0&&versions[1]===0);
  stage='real resource race';const firstMutation=clients[0].evaluate(`__chronosMultiTab.mutate({resourceId:${JSON.stringify(resource)},operationId:'browser_a',expectedVersion:0},async()=>{await new Promise((resolve)=>setTimeout(resolve,250));return 'a';}).then((x)=>({ok:true,version:x.version})).catch((e)=>({ok:false,code:e.code}))`);
  await delay(30);const locked=await clients[1].evaluate(`__chronosMultiTab.mutate({resourceId:${JSON.stringify(resource)},operationId:'browser_b',expectedVersion:0},async()=>true).then((x)=>({ok:true,version:x.version})).catch((e)=>({ok:false,code:e.code,userMessage:e.userMessage}))`);const winner=await firstMutation;
  ...
  const stale=await clients[1].evaluate(`__chronosMultiTab.mutate({resourceId:${JSON.stringify(resource)},operationId:'browser_b',expectedVersion:0},async()=>true).then((x)=>({ok:true})).catch((e)=>({ok:false,code:e.code,userMessage:e.userMessage}))`);
  check('stale browser write returns a user-facing conflict',!stale.ok&&stale.code==='TEMPORAL_VERSION_CONFLICT'&&!!stale.userMessage);
  stage='close coordinators';await Promise.all(clients.map((client)=>client.evaluate(`__chronosMultiTab.close();true`)));check('multi-tab runtime has no console or network errors',clients.every((client)=>client.errors.length===0));clients.forEach((client)=>client.close());clearTimeout(watchdog);
