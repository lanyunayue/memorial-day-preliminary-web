...
(async function(){
  ...
  assert(confirmResults.filter(Boolean).length===1,'only one tab commits the shared draft: '+JSON.stringify(confirmResults));checks.push('only one tab commits the shared draft');
